...
4. **放入 `mods`**：为该版本装好加载器后，将模组文件放入对应版本的 `**mods**` 文件夹；若无此文件夹，可先启动一次带加载器的游戏，或通过启动器打开「版本目录」查看路径。
...
3. **默认按 `V` 键**打开语音界面（**选项 → 控制** 中可搜索 `voice` / `Simple Voice Chat` 修改）。
